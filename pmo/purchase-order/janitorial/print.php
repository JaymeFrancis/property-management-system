...
if(isset($_GET['print'])){
    $id = $_GET['print'];
    $query0 = "SELECT * FROM tbl_purchase_janitorial WHERE id = '$id'";
    $result0 = mysqli_query($conn, $query0);
    $dates = mysqli_fetch_assoc($result0);

    if(mysqli_num_rows($result0)>0){
        $pdf -> setFont('Arial', 'BU', 14);
        $pdf ->Cell(190,10,"NEED TO REQUEST FOR JANITORIAL SUPPLIES (".$dates['month']." ".$dates['year'].")",0,1,'C');
        $pdf ->Ln(5);
        ...
        $query = "SELECT * FROM tbl_purchaseitems_janitorial WHERE id = '$id'";
