...
$pdf ->SetTitle('Janitorial Issuance Report');

if(isset($_GET['ref'])){
$id = $_GET['ref'];

$stmt = "SELECT * FROM janitorial_issuance_report WHERE id = '$id'";
...
$pdf ->Cell(110,5,"",0,0,'C');
$pdf ->Cell(110,5,"MONTHLY JANITORIAL ISSUANCE REPORT",0,1,'C');
$pdf ->Ln();
...
$pdf ->Cell(110,5,'',0,0);
$pdf ->Cell(50,5,'',0,0);
$pdf ->Cell(60,5,'JANITORIAL INVENTORIES',0,1,'R');
...
//TABLE CONTENT
$querySir = "SELECT * FROM tbl_request_janitorial WHERE status = 'Issued' ";
$resultSir = mysqli_query($conn, $querySir);

if(mysqli_num_rows($resultSir) > 0){
    foreach($resultSir as $ris){
        $queryRis = "SELECT *, quantityReq*price AS amount FROM tbl_ris_janitorial WHERE iss_no = '$ris[iss_no]' ";
        ...
        if(mysqli_num_rows($resultRis) > 0){
            foreach($resultRis as $items){
                ...
                $pdf ->Cell(20,8,number_format((float)$items['price'], 2, '.', ','),1,0,'C');
        ...
        $pdf ->Cell(15,8,'',1,0,'C');
        $pdf ->Cell(15,8,'',1,0,'C');
        $pdf ->Cell(30,8,'TOTAL AMOUNT:','LTB',0,'R');
            $totalStmt = "SELECT SUM(quantityReq*price) AS total FROM tbl_ris_janitorial WHERE iss_no = '$ris[iss_no]'";
            $totalQuery = mysqli_query($conn, $totalStmt);
            $total = mysqli_fetch_assoc($totalQuery);
        $pdf ->Cell(30,8,'Php. '.number_format((float)$total['total'], 2, '.', ','),'TRB',0,'C');
        $pdf ->Cell(50,8,'',1,1,'C');
