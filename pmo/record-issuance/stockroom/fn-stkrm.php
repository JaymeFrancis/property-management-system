...
if (isset($_POST['logout'])){
 ...
 if(isset($_POST['newIssuance'])){
   ...
   $query = "INSERT INTO tbl_request_stockroom (recipient, office, req_date) VALUES ('$recipient', '$office[locations]', '$reqdate')";
   $result = mysqli_query($conn, $query);
   $lastID = mysqli_insert_id($conn);

   $auditDate = date('Y/m/d');
   $auditQuery = "INSERT INTO `audit_logs`(`date`, `activity`, `user`) 
                  VALUES ('$auditDate','Added Issuance for Stockroom Supplies','$_SESSION[user_lvl]')";
 ...
 if(isset($_POST['editIssuance'])){
    ...
    $query = "UPDATE tbl_request_stockroom SET recipient = '$recipient', office = '$office', req_date = '$date' WHERE iss_no = '$refid'";
    $result = mysqli_query($conn, $query);

   $auditDate = date('Y/m/d');
   $auditQuery = "INSERT INTO `audit_logs`(`date`, `activity`, `user`) 
                  VALUES ('$auditDate','Update Issuance for Stockroom Supplies','$_SESSION[user_lvl]')";
   $auditResult = mysqli_query($conn, $auditQuery);
    if($result){
 ...
 if(isset($_GET['p'])){
    $ref = $_GET['p'];
    $query = "SELECT item_code FROM tbl_stock_stockroom WHERE particulars = '$ref' AND rem_qty > 0";
    $result = mysqli_query($conn, $query);
...
if(isset($_GET['q'])){
     $ref = $_GET['q'];
     $query = "SELECT rem_qty, unit, price FROM tbl_stock_stockroom WHERE item_code = ?";
...
if(isset($_POST['addItem'])){
   ...
   if($quantityReq > $quantity){
      ...
      $query = "INSERT INTO tbl_ris_stockroom (iss_no, particulars, item_code, quantityReq, unit, price) VALUES ('$refid','$particulars', '$item_code', '$quantityReq', '$unit', '$price')";
      $result = mysqli_query($conn, $query);
      if($result){
         $query1 = "UPDATE tbl_stock_stockroom SET rem_qty = rem_qty - '$quantityReq' WHERE item_code = '$item_code'";
...
if(isset($_GET['remove'])){
   ...
   $query = "SELECT * FROM tbl_ris_stockroom WHERE id = '$id'";
   $result = mysqli_query($conn, $query);
   ...
   if($result){
      ...
      $result1 = mysqli_query($conn, $query1);
      if($result1){
         $query2 = "DELETE FROM tbl_ris_stockroom WHERE id = $id";
...
if(isset($_GET['releaseID'])){
     ...
     $query = "SELECT * FROM tbl_ris_stockroom WHERE iss_no = '$rel_id'";
     $result = mysqli_query($conn, $query);
  
     if(mysqli_num_rows($result) > 0){
        foreach($result as $issuance){
           $issuanceQuery = "UPDATE tbl_stock_stockroom SET issuance_qty=(issuance_qty + '$issuance[quantityReq]') WHERE item_code = '$issuance[item_code]' AND particulars = '$issuance[particulars]'";
           $issuanceResult = mysqli_query($conn, $issuanceQuery);
  
        }if($issuanceResult){
               $statusQuery = "UPDATE tbl_request_stockroom SET status = 'Issued', rcv_date = CURDATE() WHERE iss_no = '$rel_id'";
               $statusResult = mysqli_query($conn, $statusQuery);

               $auditDate = date('Y/m/d');
               $auditQuery = "INSERT INTO `audit_logs`(`date`, `activity`, `user`) 
                              VALUES ('$auditDate','Released Issuance for Stockroom Supplies','$_SESSION[user_lvl]')";
               $auditResult = mysqli_query($conn, $auditQuery);
               if($statusResult){
      ...
      $_SESSION['alert'] = "There are no Items to release!";
      $_SESSION['alertCode'] = "warning";
