    ...
    <?php include '../../../navbar.php'; ?>  
        ...
        <div class="info">
                <?php
                 if(isset($_GET['ref'])){
                    $refid = $_GET['ref'];
                    $query = "SELECT id, particulars FROM tbl_stock_janitorial WHERE uid = '$refid'";
                    ...
                    <div class="col-6 md-6 order-md-1 order-first">
                        ...
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"><a href="jtrl-stocks.php?ref=<?=$row['id']?>" style="text-decoration: none;">Stocks</a></li>
                                <li class="breadcrumb-item active" aria-current="page">Add Stock</li>
                            </ol>
                        </nav>
                    </div>
                    <div class="col-6 md-6 order-md-2 order-last">  
                        <a class="btn btn-close float-end" href="jtrl-stocks.php?ref=<?=$row['id']?>" role="button" style="border-radius: 0;"></a>
                    </div>
                </div>
            </div>
                <?php
            ...
            <div class="container">
                ...
                if(isset($_GET['ref'])){
                    $refid = $_GET['ref'];
                    $query = "SELECT * FROM tbl_stock_janitorial WHERE uid = '$refid'";
                    $result = mysqli_query($conn, $query);
                    $row = mysqli_fetch_assoc($result);
                ?>
            <form class="needs-validation" action="fn-jtrl.php" method="post" novalidate>
                ...
                ?>
            <button class="btn btn-primary float-end" type="submit" name="editStock">Submit</button>
            </form>
            </div>
